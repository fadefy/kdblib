...
namespace kx
{
    public class c : TcpClient
    {
        ...
        private static int[] nt = { 0, 1, 16, 0, 1, 2, 4, 8, 4, 8, 1, 0, 8, 4, 4, 8, 8, 4, 4, 4 };

        private static object[] NU = { null,false,new Guid(),null,(byte)0,Int16.MinValue,ni,nj,(Single)nf,nf,' ',"",new DateTime(0),
        ...
        public c(string host, int port, string userName)
            : this(host, port, userName, DefaultMaxBufferSize)
        {
        }

        public c(string host, int port, string userName, int maxBufferSize)
        {
            _maxBufferSize = maxBufferSize;
            Connect(host, port);
            s = this.GetStream();
            B = new byte[2 + userName.Length];
            J = 0;
            w(userName + "\x3");
            s.Write(B, 0, J);
            if (1 != s.Read(B, 0, 1))
            {
                B = new byte[1 + userName.Length];
                Connect(host, port);
                s = this.GetStream();
                J = 0;
                w(userName);
        ...
        public static void Main(string[] args)
        {
            ...
            // for (int i = 0;
            // i < 1000;
            // ++i) c.k("insert", "trade", x);
            // tm();

            //            Flip r = td(c.k("select sum price by sym from trade"));
            // O("cols: " + n(r.x));
            // O("rows: " + n(r.y[0]));
        ...
        {
            return x is Dict ? n(((Dict)x).x) : x is Flip ? n(((Flip)x).y[0]) : x is char[] ? e.GetBytes((char[])x).Length : ((Array)x).Length;
        }
        ...
        {
            return NU["test string".IndexOf(c)];
        }
        ...
        private object r()
        {
            ...
            switch (t)
            {
                case 0: object[] L = new object[n];
                    for (;
                    i < n;
                    i++) L[i] = r();
                    return L;

                case 1: bool[] B = new bool[n];
                    for (;
                    i < n;
                    i++) B[i] = rb();
                    return B;
                ...
                case 5: short[] H = new short[n];
                    for (;
                    i < n;
                    i++) H[i] = rh();
                    return H;

                case 6: int[] I = new int[n];
                    for (;
                    i < n;
                    i++) I[i] = ri();
                    return I;

                case 7: long[] J = new long[n];
                    for (;
                    i < n;
                    i++) J[i] = rj();
                    return J;

                case 8: float[] E = new float[n];
                    for (;
                    i < n;
                    i++) E[i] = re();
                    return E;

                case 9: double[] F = new double[n];
                    for (;
                    i < n;
                    i++) F[i] = rf();
                    return F;

                case 10: char[] C = e.GetChars(b, j, n);
                    j += n;
                    return C;

                case 11: String[] S = new String[n];
                    for (;
                    i < n;
                    i++) S[i] = rs();
                    return S;

                case 12: DateTime[] P = new DateTime[n];
                    for (;
                    i < n;
                    i++) P[i] = rp();
                    return P;

                case 13: Month[] M = new Month[n];
                    for (;
                    i < n;
                    i++) M[i] = rm();
                    return M;

                case 14: Date[] D = new Date[n];
                    for (;
                    i < n;
                    i++) D[i] = rd();
                    return D;

                case 15: DateTime[] Z = new DateTime[n];
                    for (;
                    i < n;
                    i++) Z[i] = rz();
                    return Z;

                case 16: KTimespan[] N = new KTimespan[n];
                    for (;
                    i < n;
                    i++) N[i] = rn();
                    return N;

                case 17: Minute[] U = new Minute[n];
                    for (;
                    i < n;
                    i++) U[i] = ru();
                    return U;

                case 18: Second[] V = new Second[n];
                    for (;
                    i < n;
                    i++) V[i] = rv();
                    return V;

                case 19: TimeSpan[] T = new TimeSpan[n];
                    for (;
                    i < n;
                    i++) T[i] = rt();
                    return T;
        ...
        private void read(byte[] b)
        {
            ...
            for (;
            i < n;
            i += j) if (0 == (j = s.Read(b, i, Math.Min(_maxBufferSize, n - i)))) throw new Exception("read");
        }
        ...
        {
            double f = rf();
            return Double.IsInfinity(f) ? (f < 0 ? za : zw) : new DateTime(qn(f) ? 0 : clampDT(10000 * (long)Math.Round(8.64e7 * f) + o));
        }
        ...
        //public static void set(object x,int i,object y){Array.set(x,i,null==private y?NU[t(x)]:y);}
        ...
        private void w(Guid g)
        {
            ...
            if (vt < 3) throw new KException("Guid not valid pre kdb+3.0");
            for (int i = 0;
            i < b.Length;
            i++) w(b[gip[i]]);
        }
        ...
        {
            if (vt < 1) throw new KException("Timespan not valid pre kdb+2.6");
            w(qn(t) ? ni : (int)(t.Ticks / 10000));
        }

        private void w(DateTime p)
        {
            if (vt < 1) throw new KException("Timestamp not valid pre kdb+2.6");
        ...
        private void w(object x)
        {
            ...
            if (t < 0) switch (t)
                {
                    ...
                    case -15: w((DateTime)x);
                        return;

                    case -16: w((KTimespan)x);
                        return;
            ...
            switch (t)
            {
                case 0: foreach (object o in (object[])x) w(o);
                    return;

                case 1: foreach (bool o in (bool[])x) w(o);
                    return;

                case 2: foreach (Guid o in (Guid[])x) w(o);
                    return;

                case 4: foreach (byte o in (byte[])x) w(o);
                    return;

                case 5: foreach (short o in (short[])x) w(o);
                    return;

                case 6: foreach (int o in (int[])x) w(o);
                    return;

                case 7: foreach (long o in (long[])x) w(o);
                    return;

                case 8: foreach (float o in (float[])x) w(o);
                    return;

                case 9: foreach (double o in (double[])x) w(o);
                    return;

                case 10: foreach (byte b in e.GetBytes((char[])x)) w(b);
                    return;

                case 11: foreach (string o in (string[])x) w(o);
                    return;

                case 12: foreach (DateTime o in (DateTime[])x) w(o);
                    return;

                case 13: foreach (Month o in (Month[])x) w(o);
                    return;

                case 14: foreach (Date o in (Date[])x) w(o);
                    return;

                case 15: foreach (DateTime o in (DateTime[])x) w(o);
                    return;

                case 16: foreach (KTimespan o in (KTimespan[])x) w(o);
                    return;

                case 17: foreach (Minute o in (Minute[])x) w(o);
                    return;

                case 18: foreach (Second o in (Second[])x) w(o);
                    return;

                case 19: foreach (TimeSpan o in (TimeSpan[])x) w(o);
                    return;
            }
        }
